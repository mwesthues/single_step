...
\textit{Exp2} originally comprised 513 maize lines representing the global maize
...
Population structure in \textit{Exp2} was examined by running a STRUCTURE
...
sets of nine different sizes from \textit{Exp2}
...
we denote as `Core'.
In the `core' set, transcriptomic data were the best single predictor for ADL,
PRO and particulaly for DMY where the predictive ability obtained with T
...
For the FAT and SUG, G was the superior predictor inside the core set.
...
\begin{figure}[H]
  ...
  data, respectively.
  Results are shown for the core set of 110 inbred lines from \textit{Exp2} for
...
from a core set size of 0.1 to 0.2/0.3 was observed, regardless of whether G
...
set compared to the core set ($\Delta(n) = 54$) was not large enough to provide
...
genotypes in \textit{Exp2} we examined how reliably single step prediction works
