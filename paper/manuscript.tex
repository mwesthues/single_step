...
the subset of individuals with full information \cite{Technow2014}.
...
In the core set, P had the highest predictive ability for starch ('STA') and 
dry matter content ('DMC').
For the other three traits, inside the core set G was the superior predictor.
...
ranging from only 10\% to to 90\% of all genotypes covered by the incomplete
...
The latter corresponds to a situation where a large number of genotypes is
being imputed and the majority of the information is derived from genomic
information. 
...
(Fig. \ref{fig:PopStructure}A, Fig. \ref{fig:InbredResults}B).
