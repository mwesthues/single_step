...
\usepackage{tensor} % tensor matrices
...
\begin{abstract}
Predicting the performance of candidates is indispensable for selection
decisions in animal and plant breeding.
Novel `omics' predictors have shown to improve upon genomics-based predictions
of some complex traits but are yet expensive to sample.
Here we show that single step prediction can be applied to a quantitative
predictor.

Using two maize datasets of inbred lines and hybrids we show that transcriptomic
data can successfully be imputed, boosting forecasts when their predictive
ability exceeds that of pedigree or genomic data.

Equipping only a subset of genotypes with `omics' predictors and imputing all
...
\citeN{Fernando2014} derived the equivalent single step marker-effect-model to 
...
($\approx$ 1 million) needs to be predicted.

% -   Objectives (quantitative predictores)
...
Our second objective is therefore to evaluate the impact of covering only a
subset of all genotypes with both, the complete and the incomplete predictor on
predictive ability, where for the remaining individuals only genomic data are
available.
...
by~\citeNP{Yang2014}.
Best linear unbiased predictors (BLUPs) were calculated for 17
traits~\cite{Guo2016} of which six are presented in this study
...
those assigned to either cluster 1 or cluster 4 in Fig.~\ref{fig:inbred-pca}.
...
\begin{bmatrix}
 ...
 \end{bmatrix},
\end{equation}
where observed feature covariates are stored in $\tensor[_I]{\mathbf{W}}{_{2}}$ while feature
covariates for genotypes not covered by the incomplete predictor
($\tensor[_{I}]{\mathbf{\hat{W}}}{_{1}} = \tensor[_C]{\mathbf{K}}{_{12}} \; \tensor[_C]{\mathbf{K}}{_{22}^{-1}} \; \tensor[_I]{\mathbf{W}}{_{2}}$)
need to be imputed.
...
\begin{align} \label{eq:mrna1}
	...
	&= \tensor[_{C}]{\mathbf{K}}{_{12}} \; \tensor[_{C}]{\mathbf{K}}{_{22}^{-1}} \; \tensor[_{I}]{\mathbf{W}}{_{2}} \;
	\tensor[_{I}]{\boldsymbol{\hat{\alpha}}} + (\mathbf{g_1} - \tensor[_{C}]{\mathbf{K}}{_{12}} \;
	\tensor[_{C}]{\mathbf{K}}{_{2}^{-1}} \;
	\tensor[_{I}]{\boldsymbol{\hat{\alpha}}}{}) \\
	&= \mathbf{\hat{g}}_1 + \boldsymbol{\epsilon},
\end{align}

where $\tensor[_{C}]{\mathbf{K}}{_{ij}}$ are partitions of
...
\tensor[_{C}]{\mathbf{K}}{_{22}^{-1}} \;
\tensor[_{C}]{\mathbf{K}}{_{21}}\sigma^{2}_{\alpha}$
\cite{Fernando2014}.
...
\mathbf{y} =
\mathbf{Xb} + \tensor[_{I}]{\mathbf{W}}{} \; \tensor[_{I}]{\boldsymbol{\alpha}} + \mathbf{U} \boldsymbol{\epsilon} + \mathbf{e},
\end{equation}
...
\begin{bmatrix}
 ...
 \end{bmatrix}
\begin{bmatrix}
  \tensor[_{I}]{\mathbf{\hat{W}}}{_{1}} \; \tensor[_{I}]{\boldsymbol{\alpha}} + \boldsymbol{\epsilon} \\
  \tensor[_{I}]{\mathbf{W}}{_{2}} \; \tensor[_{I}]{\boldsymbol{\alpha}} \\
 \end{bmatrix}
  + \mathbf{e} \\
\end{align}
...
simply requires separating the feature matrices of the two predictors into a
paternal and a maternal component, respectively.
...
\mathbf{y} &= \mathbf{Xb} +
\mathbf{Z}_{D} \;
\tensor[_{I}]{\mathbf{W}}{_{D}} \; \tensor[_{I}]{\boldsymbol{\alpha}}{_{D}} +
\mathbf{Z}_{D} \mathbf{U}_{D} \boldsymbol{\epsilon}_{D} +
\mathbf{Z}_{F} \;
\tensor[_{I}]{\mathbf{W}}{_{F}} \;
\tensor[_{I}]{\boldsymbol{\alpha}}{_{F}} +
...
A larger dataset was available to~\citeNP{Ashraf2016}, who imputed genotypes
...
\citeNP{Guo2016} used genotypes from four different subpopulations with a model
