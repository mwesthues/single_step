---
title: Single-Step Prediction with Resampling
...
# Data Preparation
We analyzed hybrid maize data from the public breeding program of the University
...
previously described in @Guo2016:
...
1.   Remove loci with a callfrequency $<0.95$.

2.   Remove loci with heterozygosity $\geq 0.05$.

3.   Remove loci with minor allele frequency $<0.05$.
...
Principal component analyses were run for the hybrid and inbred material.
The separation between the Dent and the Flint heterotic group for the hybrid
material looked as expected but for the inbred lines we found unexpected
clustering of the lines, as determined via a STRUCTURE analysis [@Pritchard2000]
and a PCA, albeit it with very low fractions of variance explained by the first
...
the data for each K, I chose K=4 to represent the population structure
(Fig. \@ref(fig:pca-structure-plot)).
...
The set of selected individuals entails individuals assigned to clusters 1 and 4
in Fig. \@ref(fig:pca-structure-plot) and excludes individuals in the tails of
the PCA plot.
...
Finally, we defined scenario `C` as a subset of scenario `B`, comprising the
same set of genotypes as `B` but including a fixed effect for the cluster to
which each genotype's ancestry was primarily assigned.
...
constitution of the set of genotypes that has only data on one out of two
predictors by generating core samples.
...
Modifed Rogers (MR) distance as the criterion [@Thachuk2009].
...
"Hybrid" or "Inbred"), `Scenario` ("A", "B" and "C" for inbred lines; "None" for
...
to be available (Fig. \@ref(fig:sampling-prediction-scheme) **a**,
Table \@ref(tab:genotype-count)).
For the variable `Core_Fraction` we artificially removed information on a
predictor for some genotypes to implore the influence of genetic space-coverage
through incomplete predictors on predictive ability.
This random subsampling procedure was repeated 20 times and stored as
`Rnd_Level2`.

```{r genotype-count, cache=TRUE}
html_table_width <- function(kable_output, width) {
  width_html <- paste0(paste0('<col width = "', width, '">'), collapse = "\n")
  sub("<table>", paste0("<table>\n", width_html), kable_output)
}

"./data/processed/predictions/predictions.RDS" %>%
  readRDS() %>%
  dplyr::mutate(Combi = substr(Interval, start = 1, stop = 3)) %>%
  dplyr::group_by(Combi) %>%
  dplyr::distinct(TST_Geno) %>%
  dplyr::count() %>%
  dplyr::rename(`Extent-Material-Scenario` = "Combi") %>%
  knitr::kable(
    x = .,
    booktabs = TRUE,
    caption = "Number of genotypes (n) for each combination of extent (F = full, C = core), material (I = inbred, H = hybrid) and scenario."
  ) %>%
  html_table_width(c(200, 100))
```


For all core-set inbred lines we applied a second, nested randomization scheme
where $n * x\%$ genotypes were declared to have both, data on the complete and
the incomplete predictor, whereas $n - n * x\%$ genotypes had only information
on the complete predictor (Fig. \@ref(fig:sampling-prediction-scheme) **b**).
Here $x\%$ denotes the fraction of all genotypes having information on both
predictors.
...
`"Material" * "Extent" * "Scenario"` when $x\% = 100$ and `20 ** 2 = 400`
sets of predictions for each combination of `"Material" * "Extent" *
"Scenario" * "Core_Fraction"` when $x\% \neq 100$.


```{r sampling-prediction-scheme, fig.show='hold', fig.cap="Prediction sampling scheme. ($\\textbf{a}$) Level 1 sampling scheme. During the first round of sampling, 70% of the genotypes were drawn at random (black symbols) and assigned to the training set (TRN) whereas phenotypes for the other 30% of genotypes (yellow symbols) were set to missing for this particular random sample. The random samples drawn at this level were subsequently denoted as s~1~ to s~20~. ($\\textbf{b}$) Level 2 sampling scheme. For maize inbred lines from the diversity panel, some genotypes had both genotypic and transcriptomic predictor data (black symbols). For each level of s from the first round of random sampling, a second round of random sub-sampling was applied where x% of the genotypes from the training set of s were declared to have both, genotypic and transcriptomic data (black symbols) whereas 100-x% of the genotypes were declared to have only genotypic information (gray symbols). This nested subsampling process was repeated 20 times and for nine different values of $x = (10\\%, 20\\%, 30\\%, \\dots, 90\\%)$. ($\\textbf{c}$) Leave-one-out cross-validation (LOOCV) scheme for maize diversity panel inbred lines. For each of the subsets from level 1 (and level 2) LOOCV was applied. Thereto, $n$ TST were generated (yellow symbols) where all genotypes eligible for TRN membership based on $\\textbf{a}$ were assigned to TRN (black symbols). ($\\textbf{d}$) LOOCV scheme for maize hybrids. This scheme is based solely on level 1 randomization in $\\textbf{a}$ and, for each level of s, places only T0 hybrids, which do not share a common parent with a TST hybrid, inside the TRN."}
knitr::include_graphics("compound_plot.svg", auto_pdf = FALSE)
```
...
```{r hybrid-plot, cache=FALSE, fig.cap="Predictive abilities for seven agronomic traits in maize hybrids based on three different predictors and combinations thereof. The bootstrapped standard error is displayed through error bars. The predictors are pedigree (P), genomic data (G) and transcriptomic data (T). Two letters indicate single step prediction where the first letter denotes the complete predictor whereas the second letter denotes the incomplete predictor. The 'Core' set of 685 hybrids entails all hybrids with records for all predictors. The 'Full' set comprises 1521 hybrids with data for P and G and the subset of 685 hybrids that additionally has information on T."}
hyb_df %>%
...
```{r inbred-plot, cache=FALSE, fig.cap="Predictive abilities for six agronomic traits in maize inbred lines based on two different predictors and their combination. The bootstrapped standard error is displayed through error bars. The predictors are genomic data (G) and transcriptomic data (T). Two letters indicate single step prediction where the first letter denotes the complete predictor and the second letter denotes the incomplete predictor. Different combinations of extent (C = 'Core' and F = 'Full') with prediction scenario (A, B, or C) are plotted separately along the y-axis where the fist letter indicates the extent of the material and the third letter denotes the scenario. The 'Core' set of 149 inbred lines entails all genotypes with records for both predictors. The 'Full' set comprises 211 genotypes with complete data for G and the subset of 149 inbred lines that additionally has information on T. Note that scenario A only entails the 110 inbred lines that belong to clusters 1 and 4 in the PCA (Fig. 3.5)."}
inbred_df %>%
...
```{r core-inbred-plot, cache=FALSE, fig.cap="Influence of the size of the subset of genotypes with both, genomic and transcriptomic data on predictive abilities. Results are shown for six agronomic traits in the core set of 149 maize inbred lines ('CIB' and 'CIC') and the core set of 110 genotypes ('CIA'), respectively. The fraction of genotypes with information on both predictors is denoted as 'Core Fraction' and displayed on the x-axis."}
inbred_df %>%
