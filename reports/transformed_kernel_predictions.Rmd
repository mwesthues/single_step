...
trait_order <- levels(log_file$Trait)
log_file <- log_file %>%
  select(-Trait) %>%
  distinct()

pred_lst <- lapply(seq_len(nrow(log_file)), FUN = function(i) {
  job_id <- log_file[i, Job_ID, ]
  pred_obj <- readRDS(paste0("./data/derived/predictions/", job_id, ".RDS"))
  geno_number <- pred_obj %>%
    .$Geno %>%
    unique() %>%
    length()
  geno_trait_labels <- rep(trait_order, times = geno_number)
  pred_obj$Trait <- geno_trait_labels
  pred_obj$Predictor <- log_file[i, Predictor, ]
  pred_obj 
})
...
pred_order <- c("snp42-none-none", "mrna42-none-none",
                "snp100-none-none", "ped100-snp77-none",
