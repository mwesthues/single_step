...
(i.e. "xx100").
```{r Distribution-y-yhat, cache = TRUE}
...
distr_plot <- pred_df %>%
  ...
  geom_freqpoly() +
  scale_color_manual(values = viridis(length(legend_labs), option = "C"),
                     labels = legend_labs) +
...
```{r Coefficient-of-Variation-Plot, cache = TRUE}
cv_plot <- overview_df %>%
  filter(Predictor != "snp77-none-none") %>%
  mutate(Predictor = fct_relevel(Predictor, pred_order)) %>%
...
```{r Predictive-Ability-Plots, cache = TRUE}
pred_plot <- overview_df %>%
  as_tibble() %>%
  filter(Predictor != "snp77-none-none") %>%
  mutate(Predictor = fct_relevel(Predictor, pred_order),
